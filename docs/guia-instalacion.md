...
| Token rechazado (`token incorrecto` en el log) | 1) Compruebe el token **del servidor que escucha en el puerto 5050**, no solo el que muestra el instalador: `Get-Content C:\CiberMensajeria\config.json \| ConvertFrom-Json \| Select-Object token`. Debe ser `lab-lan-ciber-mensajeria`. 2) Si arrancó el servidor a mano con `py -m servidor --config config.json` desde otra carpeta (p. ej. el clon del escritorio), puede estar usando **otro** `config.json` (token antiguo o `cambia-este-token`). Use `C:\CiberMensajeria\iniciar-servidor.ps1`. 3) Si la **tarea programada** sigue corriendo con una instalación anterior, los agentes conectan a esa instancia (token viejo). Deténgala: `Stop-ScheduledTask -TaskName CiberMensajeriaServidor`. 4) En el agente Linux: `grep token /etc/ciber-mensajeria/agente.json`. |
